...
namespace Tokyo {
    ...
    void InfinityState::Init() {
        ...
        this->_Player1->set_board_ptr(_InfinityBoard.get());
        this->_Player2->set_board_ptr(_InfinityBoard.get());
    ...
    void InfinityState::HandleInput() {
        if(_gameOverClock.getElapsedTime().asSeconds() >= GAMEOVER_DELAY){
            if(_p1) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Infinity, Winner::p1, _isMute)), true);
            else if(_p2) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Infinity, Winner::p2, _isMute)), true);
            else if(_draw) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Infinity, Winner::draw, _isMute)), true);
        }
        ...
        while ( auto event = this->_data->window.pollEvent() ) {
            ...
            if(!_p1 && !_p2 && !_draw){
                if(this->_data->input.isSpriteClicked(*this->_pauseButton, sf::Mouse::Button::Left, this->_data->window)){
                    ...
                    this->_data->machine.AddState(StateRef (new PauseState(this->_data, GameID::Infinity, _isMute)), false);
                }
