...
namespace Tokyo {
    ...
    SUSState::SUSState ( GameDataRef data , PlayerType playerType, bool isMute ) : _data( data ) , _playerType( playerType ), _isMute( isMute ) {}
            ...
            if(!_p1 && !_p2 && !_draw){ 
                ...
                if(_playerType != PlayerType::COMPUTER || _currentPlayer == _Player1.get()){
                    if(this->_data->input.isSpriteClicked( *this->_grid, sf::Mouse::Button::Left, this->_data->window )){
                        ...
                        if(_susBoard->get_cell(_row, _col) == ' ' && _clock.getElapsedTime().asMilliseconds() >= 150){
                    ...
                    else if(_susBoard->get_cell(_row, _col) != ' ' && !_isMute && _clock.getElapsedTime().asMilliseconds() >= 150) this->_wrong->play();
                    }
                }
            }
        }
