...
namespace Tokyo {
    ...
    void MisereState::Init() {
        ...
        this->_Player1->set_board_ptr(_misereBoard.get());
        this->_Player2->set_board_ptr(_misereBoard.get());
                ...
                if(_playerType != PlayerType::COMPUTER || _currentPlayer == _Player1.get()){
                    if(this->_data->input.isSpriteClicked( *this->_grid, sf::Mouse::Button::Left, this->_data->window )){
                        ...
                        if (_misereBoard->get_cell(_row, _col) == ' '){
                            ...
                            if(_misereBoard->is_lose(_currentPlayer)){
                                if(_currentPlayer == _Player1.get()) _p2 = true;
                                else _p1 = true;
                            }

                            if(_misereBoard->is_draw(_currentPlayer)){
                                _draw = true;
                            }

                            if(_playerType == PlayerType::HUMAN && !_misereBoard->game_is_over(_currentPlayer)) _currentPlayer = (_currentPlayer == _Player1.get()) ? _Player2.get() : _Player1.get();
                            else if(!_misereBoard->game_is_over(_currentPlayer)) _currentPlayer = _Player2.get();
                            _clock.restart();
    ...
    void MisereState::Update ( float dt ) {
        ...
        if (_playerType == PlayerType::COMPUTER && !_misereBoard->game_is_over(_currentPlayer) && _currentPlayer == _Player2.get() && _clock.getElapsedTime().asSeconds() >= 1){
            int x, y;
            do {
                x = rand() % 3;
                y = rand() % 3;
            } while (this->_misereBoard->get_board_matrix()[x][y] != ' ');
            ...
            else if(_misereBoard->is_lose(_currentPlayer)){
                _p1 = true;
            }

            else if(_misereBoard->is_draw(_currentPlayer)){
                _draw = true;
            }

            if(!_misereBoard->game_is_over(_currentPlayer)) this->_currentPlayer = _Player1.get();
