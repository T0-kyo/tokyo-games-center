...
namespace Tokyo {
    ...
    ObstaclesState::ObstaclesState ( GameDataRef data , PlayerType playerType, bool isMute ) : _data( data ), _playerType( playerType ), _isMute( isMute ) {}
    ...
    void ObstaclesState::HandleInput() {
        if(_gameOverClock.getElapsedTime().asSeconds() >= GAMEOVER_DELAY){
            if(_p1) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Obstacles, Winner::p1, _isMute)), true);
            else if(_p2) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Obstacles, Winner::p2, _isMute)), true);
            else if(_draw) this->_data->machine.AddState(StateRef (new GameOverState(this->_data, GameID::Obstacles, Winner::draw, _isMute)), true);
        }
        ...
        while ( auto event = this->_data->window.pollEvent() ) {
            ...
            if(!_p1 && !_p2 && !_draw){
                if(this->_data->input.isSpriteClicked(*this->_pauseButton, sf::Mouse::Button::Left, this->_data->window)){
                    ...
                    this->_data->machine.AddState(StateRef (new PauseState(this->_data, GameID::Obstacles, _isMute)), false);
                }
                ...
                if(_playerType != PlayerType::COMPUTER || _currentPlayer == _Player1.get()){
                    if(this->_data->input.isSpriteClicked( *this->_grid, sf::Mouse::Button::Left, this->_data->window )){
                        ...
                        if (_obstaclesBoard->get_cell(_row, _col) == ' ' && _clock.getElapsedTime().asMilliseconds() >= 150){
                        ...
                        else if(_obstaclesBoard->get_cell(_row, _col) != ' ' && !_isMute && _clock.getElapsedTime().asMilliseconds() >= 150) this->_wrong->play();
                    }
                }
            }
        }
